...
async function main() {
	...
	// Step 1: Create users and collect their auth cookies
	...
	for (const user of users) {
		...
		if (data.errorCode === 1) {
			console.log(`  User "${user.username}" already exists — logging in instead`);
			const loginResult = await postJSON('/login', { username: user.username, password: user.password });
			const loginCookie = extractAuthToken(loginResult.setCookie);
			cookies.push(loginCookie);
			console.log(`  Logged in as "${user.username}" ${loginCookie ? '✓' : '✗'}`);
		} else if (status !== 200) {
	...
	// Step 2: Create events, one per user
