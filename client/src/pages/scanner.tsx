...
import { SignupErrors } from "../constants/interfaces";
import { useFetch } from "../hooks/fetch";
import { useAppNavigation } from "../hooks/navigation";
import { useToast } from "../hooks/use-toast";

export const QRScanner = () => {
	const { goToHome, goToEventQR } = useAppNavigation();
	...
	const fetchSignupWithEventID = useCallback(async (eventID: string) => {
		...
		if (result != null) {
			if (result?.errorCode === SignupErrors.ALREADY_SIGNED_UP) {
				goToHome();
				setTimeout(() => addToast('You are already signed up for this event.', 'info'), 0);
				return;
			} else if (result?.errorCode) {
				goToHome();
				setTimeout(() => addToast('Failed to sign up for event. Please try again.', 'error'), 0);
				return;
			} else if (result?.data?.done) {
				goToEventQR(eventID);
				setTimeout(() => addToast('Successfully signed up!', 'success'), 0);
				return;
			}
		}

		goToHome();
	}, [fetchData, goToHome, goToEventQR, addToast]);
	...
	}, [fetchSignupWithEventID, loading, stopScanner]);
